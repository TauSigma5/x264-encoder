...
StatusCode DummyContainer::WriteVideo(uint32_t p_TrackIdx, HostBufferRef* p_pBuf)
{
    ...
    p_pBuf->GetDouble(pIOPropDuration, duration);
...
StatusCode DummyContainer::WriteAudio(uint32_t p_TrackIdx, HostBufferRef* p_pBuf)
{
    ...
    p_pBuf->GetINT64(pIOPropDuration, duration);
    ...
    {
        // put the writing code here
        g_Log(logLevelWarn, "Dummy Container Plugin :: Write Audio of %ld for track %d: pts: %lld, dts: %lld, duration: %f", bufSize, p_TrackIdx, pts, dts, duration);
        p_pBuf->UnlockBuffer();
    }
